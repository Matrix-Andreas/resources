...
knitr::opts_knit$set(root.dir = "/Users/andreas/Desktop/Cognitive Science/2. Semester/Methods 2/resources/classes")
```
...
- That is to make up your own data
...
my_simulated_df <- data_frame(my_simulated_data) 

?set.seed()

```

## Ex. 0.c) Talk with your group and note in 1-2 sentences: What do we know of the data drawn from the rnorm()? There is 20 data points, with a mean of 1.5 and a sd of 2. This should be normal distributed since it's data drawn from the rnorm function.
...
```{r}
plot_sim <- ggplot(my_simulated_df, aes(x=my_simulated_data)) + geom_histogram( color = "orange", fill = "blue")
...
```
We get different values for the mean and sd that we initially put. This is because of our limited amount of data points and rnorm has to compute the random numbers to be normally distributed so with more data points it would gradually become closer to out intial stated mean and sd values. 
...
ex1_y <- intercept+slope*ex1_x

y_values <- rnorm(ex1_y, 0, error) 

df <- data_frame(ex1_x, y_values) #create data_frame of generated x and y values

#plotting

# Basic plot with ggplot2
df_plot <- ggplot(df, aes(x = ex1_x, y = y_values)) +
  geom_point() +  # Add points
  theme_minimal() +  # Use a minimal theme
  theme(
    plot.title = element_text(hjust = 0.5),  # Center the title
